...
use ast::{self, Item};
use grammar::{block, bool, value, Code};
use helpers::{ident, prefix, semi, string};
...
fn error_page<'a>() -> impl Parser<TokenStream<'a>, Output = Item> {
    ...
    let is_eq = |val: &Value| -> Result<bool, Error<_, _>> { Ok(lit(val)?.starts_with('=')) };
...
fn limit_except<'a>() -> impl Parser<TokenStream<'a>, Output = Item> {
    ident("limit_except")
        ...
        .map(|(methods, (position, directives))| {
            Item::LimitExcept(ast::LimitExcept {
                methods,
                position,
                directives,
            })
...
pub fn directives<'a>() -> impl Parser<TokenStream<'a>, Output = Item> {
    choice((
        ...
        ident("recursive_error_pages")
            .with(bool())
            .skip(semi())
            .map(Item::RecursiveErrorPages),
        ident("chunked_transfer_encoding")
            .with(bool())
            .skip(semi())
