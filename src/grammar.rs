...
use combine::parser::combinator::{no_partial, opaque, FnOpaque};
use combine::{choice, position};
use combine::{eof, many, many1, Parser};

use ast::{self, Directive, Item, Main};
use error::ParseError;
use helpers::{ident, semi, string, text};
use position::Pos;
...
pub fn bool<'a>() -> impl Parser<TokenStream<'a>, Output = bool> {
    choice((ident("on").map(|_| true), ident("off").map(|_| false)))
}

pub fn value<'a>() -> impl Parser<TokenStream<'a>, Output = Value> {
    (position(), string()).and_then(|(p, v)| Value::parse(p, v))
}

pub fn worker_processes<'a>() -> impl Parser<TokenStream<'a>, Output = Item> {
...
pub fn map<'a>() -> impl Parser<TokenStream<'a>, Output = Item> {
    use helpers::kind;
    use tokenizer::Kind::{BlockEnd, BlockStart};
    enum Tok {
        Hostnames,
        Volatile,
...
pub fn block<'a>() -> FnOpaque<TokenStream<'a>, ((Pos, Pos), Vec<Directive>)> {
    use helpers::kind;
    use tokenizer::Kind::{BlockEnd, BlockStart};
    opaque(|f| {
        f(&mut no_partial((
...
impl Code {
    pub fn parse<'x, 'y>(code_str: &'x str) -> Result<Code, Error<Token<'y>, Token<'y>>> {
    ...
    ident("try_files")
        ...
        .and_then(|mut v: Vec<_>| -> Result<_, Error<_, _>> {
            ...
            let last = match &last.data[..] {
                [Literal(x)] if x.starts_with("=") => Code(self::Code::parse(&x[1..])?.as_code()),
                [Literal(x)] if x.starts_with("@") => NamedLocation(x[1..].to_string()),
                _ => Uri(last.clone()),
            };
...
pub fn openresty<'a>() -> impl Parser<TokenStream<'a>, Output = Item> {
    use ast::Item::*;
    choice((
        ident("rewrite_by_lua_file")
            .with(value())
            .skip(semi())
            .map(Item::RewriteByLuaFile),
        ident("balancer_by_lua_file")
            .with(value())
            .skip(semi())
            .map(BalancerByLuaFile),
        ident("access_by_lua_file")
            .with(value())
            .skip(semi())
            .map(AccessByLuaFile),
        ident("header_filter_by_lua_file")
            .with(value())
            .skip(semi())
            .map(HeaderFilterByLuaFile),
        ident("content_by_lua_file")
            .with(value())
            .skip(semi())
            .map(ContentByLuaFile),
        ident("body_filter_by_lua_file")
            .with(value())
            .skip(semi())
            .map(BodyFilterByLuaFile),
        ident("log_by_lua_file")
            .with(value())
            .skip(semi())
            .map(LogByLuaFile),
        ident("lua_need_request_body")
            .with(value())
            .skip(semi())
            .map(LuaNeedRequestBody),
        ident("ssl_certificate_by_lua_file")
            .with(value())
            .skip(semi())
            .map(SslCertificateByLuaFile),
        ident("ssl_session_fetch_by_lua_file")
            .with(value())
            .skip(semi())
            .map(SslSessionFetchByLuaFile),
        ident("ssl_session_store_by_lua_file")
            .with(value())
            .skip(semi())
...
pub fn directive<'a>() -> impl Parser<TokenStream<'a>, Output = Directive> {
    position()
        .and(choice((
            ident("daemon").with(bool()).skip(semi()).map(Item::Daemon),
            ident("master_process")
                .with(bool())
                .skip(semi())
            ...
            ident("http")
                .with(block())
                .map(|(position, directives)| ast::Http {
                    position,
                    directives,
                })
                .map(Item::Http),
            ident("server")
                .with(block())
                .map(|(position, directives)| ast::Server {
                    position,
                    directives,
                })
                .map(Item::Server),
            ...
            ident("include")
                ...
                .skip(semi())
                .map(Item::Include),
            ident("ssl_certificate")
                .with(value())
                .skip(semi())
                .map(Item::SslCertificate),
            ident("ssl_certificate_key")
                .with(value())
                .skip(semi())
...
pub fn parse_directives(s: &str) -> Result<Vec<Directive>, ParseError> {
    ...
    let (doc, _) = many1(directive())
        ...
        // TODO: How to fix that ? Gne Gne Gne on change des trucs XD. Oui je suis français et alors ?
