...
pub fn directives<'a>() -> impl Parser<TokenStream<'a>, Output = Item> {
    choice((
        ...
        ident("proxy_cache_valid")
            ...
            .skip(semi())
            .map(Item::ProxyCacheValid),
        ident("proxy_read_timeout")
            .with(value())
            .skip(semi())
            .map(Item::ProxyReadTimeout),
        ident("proxy_connect_timeout")
            .with(value())
            .skip(semi())
            .map(Item::ProxyConnectTimeout),
        ident("proxy_hide_header")
            .with(value())
            .skip(semi())
            .map(Item::ProxyHideHeader),
        ident("proxy_pass_header")
            .with(value())
            .skip(semi())
            .map(Item::ProxyPassHeader),
        ident("proxy_pass_request_headers")
            .with(bool())
            .skip(semi())
            .map(Item::ProxyPassRequestHeaders),
        ident("proxy_pass_request_body")
            .with(bool())
            .skip(semi())
            .map(Item::ProxyPassRequestBody),
        ident("proxy_intercept_errors")
            .with(bool())
            .skip(semi())
            .map(Item::ProxyInterceptErrors),
        ident("proxy_buffering")
            .with(bool())
            .skip(semi())
            .map(Item::ProxyBuffering),
        ident("proxy_ignore_headers")
            .with(many1(string()))
            .skip(semi())
            .map(|v: Vec<_>| v.into_iter().map(|v| v.value.to_string()).collect())
        ...
        ident("proxy_next_upstream_tries")
            .with(value())
            .skip(semi())
            .map(Item::ProxyNextUpstreamTries),
        ident("proxy_next_upstream_timeout")
            .with(value())
            .skip(semi())
