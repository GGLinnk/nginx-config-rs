...
use combine::Parser;
use combine::{choice, many1, optional, position};

use ast::{self, Item};
use grammar::{block, value, Code};
        ...
        .and_then(|(a, b)| -> Result<_, Error<_, _>> {
            if let Some(target) = b {
                match Code::parse(lit(&a)?)? {
                    Code::Redirect(code) => Ok(Redirect {
                        code: Some(code),
                        url: target,
                    }),
                    Code::Normal(code) => Ok(Text {
                        code: code,
                        text: Some(target),
                    }),
                }
                ...
                match a.data.get(0) {
                    Some(Literal(x)) if x.starts_with("https://") || x.starts_with("http://") => {
                        Ok(Redirect {
                            code: None,
                            url: a.clone(),
                        })
                    }
                    Some(Variable(v)) if v == "scheme" => Ok(Redirect {
                        code: None,
                        url: a.clone(),
                    }),
...
fn strip_open_paren<'a>(v: &'_ mut Vec<&'a str>) -> Result<(), Error<Token<'a>, Token<'a>>> {
    match v.get_mut(0) {
        Some(s) => {
            if s.starts_with('(') {
                if &s[..] != "(" {
...
fn strip_close_paren<'a>(v: &'_ mut Vec<&'a str>) -> Result<(), Error<Token<'a>, Token<'a>>> {
    match v.last_mut() {
        Some(s) => {
            if s.ends_with(')') {
                if &s[..] != ")" {
...
fn if_directive<'a>() -> impl Parser<TokenStream<'a>, Output = Item> {
    ident("if")
        ...
        .and(block())
        .map(|(condition, (position, directives))| ast::If {
            position,
            condition,
            directives,
        })
